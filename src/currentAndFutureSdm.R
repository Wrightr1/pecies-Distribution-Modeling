...
# get our occurence data, just lat/lon
occurrenceCoords<-read_csv("data/cleanedData.csv") %>%
  dplyr::select( decimalLongitude, decimalLatitude)

# convert to spatial points for models
...
# drill down where our data lives

geographicExtent <- extent(x = occurrenceSpatialPts)


# standardize random points for reproducability
...
# create a grid of climate measurements, per occurence point
...
# create a vector of presence/absences
presenceAbsenceV <- c(rep(1, nrow(occEnv)), rep(0, nrow(absenceEnv))) 

# create a single data frame of presence/absence data by climate variable
